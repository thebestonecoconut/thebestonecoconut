...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$raw=Get-Content -LiteralPath '%~f0' -Raw; $m=[char]35+'PSSTART'; $i=$raw.IndexOf($m); $code=$raw.Substring($i+$m.Length); [IO.File]::WriteAllText($env:TMPPS,$code,(New-Object System.Text.UTF8Encoding($false)))"
